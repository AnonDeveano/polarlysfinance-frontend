...
import { NEAR_TICKER, NEBULA_TICKER, BOREALIS_TICKER, ZAPPER_ROUTER_ADDR } from '../utils/constants';
...
function useApproveZapper(zappingToken: string): [ApprovalState, () => Promise<void>] {
  ...
  let token: ERC20;
  if (zappingToken === NEAR_TICKER) token = polarlysFinance.NEAR;
  else if (zappingToken === NEBULA_TICKER) token = polarlysFinance.NEBULA;
  else if (zappingToken === BOREALIS_TICKER) token = polarlysFinance.BOREALIS;
  ...
  const approvalState: ApprovalState = useMemo(() => {
    ...
    if (token === polarlysFinance.NEAR) return ApprovalState.APPROVED;
  ...
  }, [currentAllowance, pendingApproval, token, polarlysFinance]);
